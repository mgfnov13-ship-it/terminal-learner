# -*- coding: utf-8 -*-
"""
atelier-data core - BM25 search engine over curated design datasets.

Derived from UI/UX Pro Max v2.5.0 (core.py), (c) 2024 Next Level Builder, MIT.
See ../references/PROVENANCE.md and ../../THIRD-PARTY-LICENSES/ui-ux-pro-max-MIT.txt.

Changes vs upstream:
- tokenize() now keeps 2-char tokens (len >= 2) so queries like "ux"/"ai" work.
- Dropped the `icons` and `web`(app-interface, mobile-only) domains.
- STACK_CONFIG trimmed to the web stacks this suite uses (no native / no html-tailwind v3).
...
CSV_CONFIG = {
    ...
    "reasoning": {
        ...
        "output_cols": ["UI_Category", "Recommended_Pattern", "Style_Priority", "Color_Mood", "Typography_Mood", "Key_Effects", "Anti_Patterns", "Severity"]
    },
    "ux": {
...
class BM25:
    ...
    def tokenize(self, text):
        """Lowercase, split, remove punctuation, keep tokens of length >= 2."""
...
def detect_domain(query):
    ...
    domain_keywords = {
        ...
        "reasoning": ["reasoning", "anti-pattern", "anti-patterns", "decision rule", "what pattern"]
    }
